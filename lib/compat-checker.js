...
const DEPRECATED_PATTERNS = [
  ...
  { pattern: /\bFile\.new\s*\(/, message: 'File.new() → FileAccess.open()' },
  { pattern: /\bDirectory\.new\s*\(/, message: 'Directory.new() → DirAccess.open()' },
  { pattern: /\.change_scene\s*\(/, message: '.change_scene() → .change_scene_to_file()' },
  { pattern: /\bconnect\s*\(\s*["'][^"']+["']\s*,\s*self\s*,/, message: 'connect("signal", self, "_method") → signal.connect(_method)' },
  { pattern: /\.raise\s*\(/, message: '.raise() → .move_to_front()' },
  { pattern: /\bcanvas_item\.update\s*\(/, message: 'CanvasItem.update() → queue_redraw()' },
  { pattern: /\bemit_signal\s*\(/, message: 'emit_signal() → signal_name.emit()' },
  { pattern: /^onready\s+var\b/m, message: 'onready var → @onready var' },
  { pattern: /^export\s+var\b/m, message: 'export var → @export var' },
  { pattern: /\bsetget\b/, message: 'setget → use set/get property syntax (Godot 4)' },
  { pattern: /\bPool(String|Int|Real|Vector2|Vector3|Color|Byte)Array\b/, message: 'Pool*Array → Packed*Array (e.g. PackedStringArray)' },
];
...
function collectFiles(dir) {
  ...
  for (const entry of entries) {
    ...
    } else if (entry.isFile() && (entry.name.endsWith('.gd') || entry.name.endsWith('.tscn'))) {
      results.push(full);
    }
  }
  return results;
...
module.exports = { checkFile, checkString, checkDir, formatWarnings, DEPRECATED_PATTERNS, collectFiles };
