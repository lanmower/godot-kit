...
const GODOT_VERSION = readConfig().godotVersion || '4.6-stable';
...
function findGodot(explicitPath) {
  ...
  if (process.platform === 'darwin') {
    const macBin = path.join(CFG_DIR, 'Godot.app', 'Contents', 'MacOS', 'Godot');
...
async function downloadEngine() {
  ...
  let godotPath = files.length ? path.join(CFG_DIR, files[0]) : null;
  if (!godotPath && process.platform === 'darwin') {
    const macBin = path.join(CFG_DIR, 'Godot.app', 'Contents', 'MacOS', 'Godot');
    if (fs.existsSync(macBin)) godotPath = macBin;
  }
