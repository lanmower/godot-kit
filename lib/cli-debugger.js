...
function registerDebuggerCommands(program) {
  ...
  program.command('attach').description('Auto-detect running Godot (TCP port 6007 or HTTP port 6009) and start REPL')
    ...
    .action(async (opts) => {
      ...
      if (conn.transport === 'tcp') {
        ...
        const pj = (o) => console.log(JSON.stringify(o, null, 2));
        const SPECIAL = {
          '.tree': async () => { try { pj(await conn.tree()); } catch(e) { console.error(e.message); } },
          '.perf': async () => { try { pj(await conn.perf()); } catch(e) { console.error(e.message); } },
          '.globals': async () => { try { pj(await conn.globals()); } catch(e) { console.error(e.message); } },
          '.help': () => console.log('Commands: .tree .perf .globals .help exit or any GDScript expression'),
        };
        rl.prompt();
        rl.on('line', async (line) => {
          ...
          if (SPECIAL[expr]) { await SPECIAL[expr](); rl.prompt(); return; }
          try {
            const r = await conn.eval(expr);
            if (r && r.error) console.error('\x1b[31m' + r.error + '\x1b[0m');
            else console.log(r && r.result !== undefined ? (r.result === '' ? '(null)' : r.result) : JSON.stringify(r));
  ...
  program.command('inspect').description('Dump scene tree from running Godot game (one-shot)')
    .option('--host <host>', 'Godot host', '127.0.0.1')
    .option('-p, --port <port>', 'Debugger port', '6007')
    .option('--timeout <ms>', 'Max wait for scene tree (ms)', '5000')
    .action(async (opts) => {
      ...
      setTimeout(() => { if (!done) { console.error('Timeout waiting for scene tree.'); client.disconnect(); process.exit(1); } }, parseInt(opts.timeout));
    });
